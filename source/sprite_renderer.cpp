...
void SpriteRenderer::DrawSprite(Sprite *sprite) {
    ...
    this->shader.Use();
    glm::mat4 model = glm::mat4(1.0f);
    model = glm::translate(model, glm::vec3(sprite->position,
    ...
    model = glm::translate(model, glm::vec3(0.5f * sprite->size.x, 0.5f * sprite->size.y,
                                            0.0f));   // move origin of rotation to center of quad
    model = glm::rotate(model, glm::radians(sprite->rotate), glm::vec3(0.0f, 0.0f, 1.0f));          // then rotate
